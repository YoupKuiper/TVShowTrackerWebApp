...
const App = () => {
  ...
  const showTVShowDetailsModal =
    tvShowDetailsToShow && tvShowDetailsToShow.id !== DEFAULT_TV_SHOW.id;
  const queryTrackedTVShows = useQuery(
    ["tracked", searchTracked],
    () => getTrackedTVShows(searchTracked),
    { enabled: isLoggedIn, staleTime: 60000 }
  );
    ...
    try {
      ...
      if (toRemove) {
        newTrackedTvShowsList = new Set(
          queryTrackedTVShows.data.filter(
            (trackedTVShow) => trackedTVShow.id !== tvShow.id
          )
        );
      } else {
        newTrackedTvShowsList = new Set(
          queryTrackedTVShows.data.concat(tvShow)
        );
      }
  ...
  const AppView = () => {
    ...
    return (
      <div
        className={
          darkMode
            ? "tst-main-div dark bg-gray-800 w-full h-screen text-white"
            : ""
        }
      >
        <ToastContainer />
        <NavBar
          currentPage={
            isTrackedList ? PAGE_NAME_TRACKED_TV_SHOWS : PAGE_NAME_SEARCH
          }
          darkMode={darkMode}
        ...
        {!isLoggedIn && (
          <div className="container mx-auto text-center py-5">
            ...
            <div className="text-lg">
              Add shows to your Watchlist and get email notifications when
              episodes air!
            </div>
            <div className="text-lg">
              <button
        ...
        <SearchBar
          search={
            isTrackedList
              ? () => setSearchTracked(searchTerm)
              : () => setSearchPopular(searchTerm)
          }
        ...
        <TVShowsListView
          ...
          handleButtonClick={
            isTrackedList ? removeTrackedTVShow : addTrackedTVShow
          }
          isLoggedIn={isLoggedIn}
    ...
    <>
      <Routes>
        ...
        <Route path="/unsubscribe" element={<UnsubscribeEmailModal />}>
          <Route
            path=":emailAddress/:token"
            element={<UnsubscribeEmailModal />}
          />
        </Route>
