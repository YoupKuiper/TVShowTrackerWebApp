...
import { LoginResponse, LoginUser, LoginUserObject, PasswordResetEmail, SimpleResponse, User } from "../../validators";
...
const LoginFormModal = ({ setShowLoginModal, loginUser, createAccount, setLoggedInUser }: LoginFormModalProps) => {
	...
	const handleSendPasswordResetEmail = async (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
		...
		try {
			...
			const { data, status } = await axios.post<SimpleResponse>(`${TV_SHOW_TRACKER_API_BASE_URL}/ResetPassword`, {
				emailAddress,
			});
	...
	return (
      ...
      flex justify-center items-center tst-loginModal"
			>
				<div className="bg-white max-w-md w-full space-y-8 p-10 rounded-md dark:bg-gray-700 dark:text-white">
					...
					<div style={{ marginTop: "0" }}>
						...
						<h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">
							{showPasswordResetForm ? "Please enter your email address" : "Sign in to your account"}
						</h2>
					</div>
					{showSpinner || userMutation.isLoading ? (
							...
							<div>
								{!message ? (
									<button
										...
										className="tst-formSendButton group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
									>
									...
									<div className="text-center">
										<b>{message.toString()}</b>
									</div>
								)}
							</div>
							<div className="text-center">
								Don't have an account yet?{" "}
								<button className="underline" onClick={() => createAccount()}>
									Create one!
								</button>
							</div>
						</form>
