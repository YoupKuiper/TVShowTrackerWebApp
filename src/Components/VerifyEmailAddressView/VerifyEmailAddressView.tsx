...
export default function VerifyEmailAddressView() {
    ...
    const callVerifyEmailAddressEndpoint = async () => {
        const { data } = await axios.post<AxiosResponse<string>>(
            ...
            { emailAddress: params.emailAddress, verifyEmailAddressToken: params.token }
        );
        return data;
    }
    ...
    useEffect(() => {
        setShowSpinner(true)
        callVerifyEmailAddressEndpoint().then((data) => {
      ...
      flex justify-center items-center" >
                <div className="bg-white max-w-md w-full space-y-8 p-10 rounded-md dark:bg-gray-700 dark:text-white" >
                    ...
                    <div style={{ marginTop: "0" }}>
                        ...
                        />
                        {success ? <h2>Done!</h2>
                        : <h2 className="mt-6 text-center text-md font-extrabold text-gray-900 dark:text-white">Verifying your email address</h2>}
                    </div>
