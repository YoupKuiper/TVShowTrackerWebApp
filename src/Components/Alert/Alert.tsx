...
interface AlertProps {
    message: string
    index: number
    closeAlert: (index: number) => any;
}

export const Alert = ({ message, index, closeAlert }: AlertProps) => {
        ...
        <>            
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                ...
                <span className="absolute top-0 bottom-0 right-0 px-4 py-3">
                    <svg className="fill-current h-6 w-6 text-red-500" role="button" onClick={() => closeAlert(index)}
