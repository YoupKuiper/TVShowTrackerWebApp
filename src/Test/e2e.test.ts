...
describe("App.js", () => {
  ...
  const expectToastAndCloseIt = async (page: Page, toastText?: string) => {
    ...
    await page.waitForSelector(".Toastify__close-button", {
      hidden: false,
      visible: true,
    });
    await delay(100);
  ...
  it("loads popular tv shows", async () => {
    ...
    await page.waitForSelector(".tst-modal-add-button", {
      visible: true,
      hidden: false,
    });
    await page.click(".tst-modal-add-button");
    await expectToastAndCloseIt(
      page,
      "Please log in first to add shows to your Watchlist"
    );
    ...
    expect(await page.evaluate((el) => el.ariaCurrent, navBarButtons[0])).toBe(
      "page"
    );
    ...
    expect(await page.evaluate((el) => el.ariaCurrent, navBarButtons[1])).toBe(
      "page"
    );
    const userButton = await page.waitForSelector(".tst-userButton");
    await userButton?.click();
    await page.waitForSelector(".tst-logged-in", {
      visible: true,
      timeout: 10000,
    });
    ...
    expect(await page.evaluate((el) => el.textContent, tvShows[0])).toBe(
      showName
    );
